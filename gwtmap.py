...
HTTP_PASS = None
HTTP_AUTH = None
SESSION = requests.Session()  # reused across requests for connection keep-alive
...
def present_dispatch_actions(actions, quiet):
    """ Prints the enumerated GWTP dispatch action classes """
    if not quiet:
        print_heading("GWTP Dispatch Actions")
    else:
        writer()

    shown = [action for action in actions if FILTER in action["actionClass"]]

    if len(shown) < 1:
        writer("No dispatch actions were identified!", FORMAT['WARNING'])
        writer()
        return 0

    writer(
        f"{len(shown)} action class(es) dispatched via "
        "DispatchService.execute()\n"
    )
    for action in shown:
        line = f"{action['actionClass']}/{action['typeHash']}"
        if action["mode"] is not None:
            line += f"  [Mode: {action['mode'].split('.')[-1]}]"
        writer(line)
    writer()
    return len(shown)
...
def synthesize_param_values(full_signature):
    """ Returns placeholder parameter values for a method signature, used when
        the values cannot be read from the code (GWT 2.7+ ServiceHelper path) """
    param_values = []
    for i, sig in enumerate(full_signature):
        if sig.startswith(COMPLEX_TYPES["BOOLEAN"]):
            param_values.append("§0§")
        elif sig in SIMPLE_TYPES or sig.startswith("I"):
            param_values.append(f"§{randint(0,99)}§")
        else:
            param_values.append(f"§param_stub{i}§")
    return param_values

def build_dispatch_rpc_call(method_object):
    """ Returns a Tuple of the best-effort GWT-RPC call structure and the list
        of complex types for a GWT 2.7+ ServiceHelper method, built from the
        recovered signature rather than from inline code values """
    full_signature = method_object["methodSignature"]
    normalised_signature = normalise_signature(full_signature)
    param_values = synthesize_param_values(full_signature)
    string_values, cmplx_types = get_string_values(param_values, full_signature)

    rpc_payload_length = str(4 + len(normalised_signature) + len(string_values))
    strong_name = (
        method_object["service"]["strongName"]
        if method_object.get("service") is not None
        else "X" * 32
    )
    rpc_blocks = [
        RPC_VERSION,
    ...
    rpc_blocks.extend(normalised_signature)
    rpc_blocks.extend(string_values)
    rpc_blocks.extend(["1", "2", "3", "4", method_object["paramCount"]])
    rpc_blocks.extend(
        generate_parameter_map(rpc_blocks, full_signature, param_values)
    )
    return rpc_blocks, cmplx_types
...
def extract_servicehelper_ctors(code):
    """ Returns a dict mapping a GWT 2.7+ 'ServiceHelper' constructor function
        name to its (serviceProxy, methodName).

    GWT 2.7 changed the generated RPC proxy: each async method is backed by a
    small constructor that hard-codes the '{Proxy}.{method}' stats identifier
    and the method name, e.g.

        function BNd(a){ this.e=a; this.a='DispatchService_Proxy.execute';
                         this.b='execute'; this.c=new gOd }

    This is what frameworks such as GWTP (gwtplatform dispatch) emit, and the
    legacy proxy pattern does not match it. """
    ctors = {}
    func_def = re.compile(rf"^function ({R_VAR})\(")
    proxy_line = re.compile(rf"this\.{R_VAR} ?= ?'([A-Za-z0-9_\.\$]+)\.([A-Za-z0-9_]+)';")
    method_line = re.compile(rf"this\.{R_VAR} ?= ?'([A-Za-z0-9_]+)';")

    for i, line in enumerate(code):
        proxy_match = proxy_line.search(line)
        if not proxy_match:
            continue

        proxy_full, method_hint = proxy_match.group(1), proxy_match.group(2)

        # The bare method-name assignment sits on a nearby line and must match
        method_name = None
        for j in range(i + 1, min(i + 4, len(code))):
            method_match = method_line.search(code[j])
            if method_match and method_match.group(1) == method_hint:
                method_name = method_match.group(1)
                break
        if method_name is None:
            continue

        # Walk upwards to find the enclosing constructor function's name
        for k in range(i, max(i - 20, -1), -1):
            func_match = func_def.search(code[k])
            if func_match:
                ctors[func_match.group(1)] = (proxy_full, method_name)
                break

    return ctors

def extract_dispatch_methods(code, service_objects):
    """ Returns a List of GWT 2.7+ RPC method objects using the 'ServiceHelper'
        serialization pattern (e.g. GWTP dispatch), which the legacy proxy
        pattern in extract_method_info does not detect.

    The method body serialises, in a single comma-expression:
        writeString(w, '<remote-service-interface>')
        writeString(w, <methodNameExpr>)
        writeInt(w, <paramCount>)
    followed by one writeString per parameter type. """
    ctors = extract_servicehelper_ctors(code)
    func_def = re.compile(rf"^function ({R_VAR})\(")

    # writeString(w,'IFACE'), writeString(w, method), writeInt(w, count)
    prepare_pattern = re.compile(
        r"(?P<ws>[A-Za-z0-9_\$]+)\((?P<w>[A-Za-z0-9_\.\$]+),"
        r"'(?P<iface>[a-z][A-Za-z0-9_\.]*\.[A-Za-z][A-Za-z0-9_]*)'\)"
        r",(?P=ws)\((?P=w),(?P<method>[^,\)]+)\)"
        r",(?P<wi>[A-Za-z0-9_\$]+)\((?P=w),(?P<count>\d+)\)"
    )
    # ''+writeString(w, <paramType>) - one per serialised parameter type
    type_pattern = re.compile(r"''\+[A-Za-z0-9_\$]+\([A-Za-z0-9_\.\$]+,([^\)]+)\)")

    method_objects = []
    for loc, line in enumerate(code):
        prep = prepare_pattern.search(line)
        if not prep:
            continue

        iface = prep.group("iface")
        param_count = int(prep.group("count"))

        # Determine the bounds of the enclosing method function
        func_start = 0
        for k in range(loc, -1, -1):
            if func_def.search(code[k]):
                func_start = k
                break
        func_end = len(code)
        for k in range(loc + 1, len(code)):
            if code[k].startswith("function "):
                func_end = k
                break

        # Resolve the method name + service proxy via the ServiceHelper ctor
        # instantiated in this function (methodExpr is e.g. 'g.b')
        helper_var = prep.group("method").split(".")[0]
        new_pattern = re.compile(rf"{re.escape(helper_var)} ?= ?new ({R_VAR})\(")
        service_proxy = method_name = None
        for k in range(func_start, loc + 1):
            new_match = new_pattern.search(code[k])
            if new_match and new_match.group(1) in ctors:
                service_proxy, method_name = ctors[new_match.group(1)]
                break

        if method_name is None:
            # Fall back to a literal method name and a derived proxy name
            method_name = prep.group("method").strip("'")
            service_proxy = f"{iface.split('.')[-1]}_Proxy"

        # Collect the serialised parameter types (resolving type variables)
        method_signature = []
        for k in range(loc + 1, func_end):
            for raw_type in type_pattern.findall(code[k]):
                method_signature.append(parse_parameter(code, raw_type.strip()))
                if len(method_signature) >= param_count:
                    break
            if len(method_signature) >= param_count:
                break
        ...
        method_object = {
            ...
            "rmtSvcIntName": iface,
            "paramCount": str(param_count),
        }

        if RPC_MODE:
            method_object["service"] = correlate_service(method_object, service_objects)
            rpc_call, ctypes = build_dispatch_rpc_call(method_object)
...
def extract_serializable_types(code):
    """ Returns a dict of {typeName: typeHash} for every serializable type
        registered in the GWT type-serializer tables.

    GWT emits these as '<fully.qualified.Name>/<typeHash>' string literals, and
    the hash is part of the RPC wire format for that type. """
    types = {}
    type_pattern = re.compile(r"'([A-Za-z][A-Za-z0-9_.\$]*)/([0-9]{4,})'")
    for line in code:
        for name, type_hash in type_pattern.findall(line):
            types.setdefault(name, type_hash)
    return types

def extract_dispatch_actions(code):
    """ Returns a List of GWTP dispatch action objects enumerated from the
        serializable-type registry.

    With the GWTP dispatch pattern there is only one RPC method
    (DispatchService.execute), and each server operation is a distinct 'Action'
    class serialised as its argument. Those action classes - the real operation
    surface - are registered as serializable types, so they can be recovered
    statically even though they are not RPC methods. """
    types = extract_serializable_types(code)

    # Map each '<Owner>$...Mode' enum back to its owning outer class
    mode_enums = {
        name.split("$")[0]: name
        for name in types
        if "$" in name and name.split("$")[-1].endswith("Mode")
    }

    actions = []
    for name, type_hash in types.items():
        if not name.endswith("Action"):
            continue
        # The action's mode enum is owned by the outer class (name minus 'Action')
        owner = name[:-len("Action")]
        actions.append({
            "actionClass": name,
            "typeHash": type_hash,
            "mode": mode_enums.get(owner),
        })

    return sorted(actions, key=lambda action: action["actionClass"])
...
def extract_method_info(code, service_objects):
    ...
    # Fall back to the GWT 2.7+ ServiceHelper pattern (e.g. GWTP dispatch) if
    # the legacy proxy pattern did not identify any methods
    if not method_objects:
        method_objects = extract_dispatch_methods(code, service_objects)
...
def set_http_params(args):
    ...
    # Size the connection pool to the fragment concurrency so keep-alive
    # connections are reused instead of re-dialled for every request
...
##################################################
def classify_response(response, fatal=True):
    """ Returns the type identifier for the provied source code.
        If fatal is False, returns None for unrecognised content instead
        of exiting (used when probing deferred fragments) """
...
def fetch_fragment(frag):
    """ Fetches a single deferred fragment. Returns a
        (frag, url, status, response) Tuple, with a None status on network
        error so the caller can treat it as a miss instead of aborting. """
    url = f"{BASE_URL}{DEFERRED}{GWT_PERMUTATION}/{frag}{F_SUFFIX}"
    try:
        response = SESSION.get(
            url, proxies=PROXIES, cookies=COOKIES, auth=HTTP_AUTH, verify=False
        )
        return frag, url, response.status_code, response.text

    except requests.exceptions.RequestException:
        return frag, url, None, ""

def append_fragments(code, code_type, args):
    """ Returns a new code List with any enumerated deferred JS fragments appended.

    Deferred fragments are numbered from 1 (fragment 0 is inlined in the
    permutation). Non-existent fragments are not guaranteed to return an HTTP
    error - many GWT servers respond with a 200 and an HTML placeholder page -
    so a fragment is only accepted once its body classifies as a GWT fragment.
    Enumeration stops after MAX_MISS consecutive non-fragment responses.

    Because the total number of fragments is not known up front, fragments are
    fetched concurrently in windows of args.threads and processed in order. """
    max_miss, miss, frag = 3, 0, 1
    threads = max(1, args.threads)

    while miss < max_miss:
        window = range(frag, frag + threads)

        # Fetch the whole window concurrently; map preserves request order
        with ThreadPoolExecutor(max_workers=threads) as pool:
            results = list(pool.map(fetch_fragment, window))

        for _, frag_url, status, response in results:
            frag_type = (
                classify_response(response, fatal=False) if response else None
            )

            if status == 200 and frag_type is not None and frag_type.startswith(FRAGMENT):
                if not args.code and not args.quiet:
                    writer(f"+ fragment : {frag_url}")

                code.extend(clean_code(response, frag_type))
                miss = 0
            else:
                miss += 1
                if miss >= max_miss:
...
def main():
    ...
    parser.add_argument(
        "--actions", action="store_true", default=False,
        help="enumerate GWTP dispatch action classes from the type registry - "
        + "the real operation surface when DispatchService is the only method"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-t", "--threads", metavar="<N>", type=int, default=10,
        help="number of concurrent requests when fetching deferred "
        + "fragments in -u/--url mode (default: 10)"
    )
    ...
    # GWTP dispatch funnels every operation through a single execute() method;
    # the real operation surface is the set of Action classes (see --actions)
    is_dispatch = any(
        method["rmtSvcIntName"].endswith("DispatchService")
        for method in method_objects
    )

    if args.actions:
        action_objects = extract_dispatch_actions(code)
        present_dispatch_actions(action_objects, args.quiet)
    elif is_dispatch and not args.quiet:
        writer(
            "Note: GWTP dispatch detected - run with --actions to enumerate "
            "the action classes (the real operation surface)\n",
            FORMAT['WARNING']
        )
